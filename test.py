...
def test(champions: list):
  problem = constraint.Problem()
  problem.addVariable('selection', champions)
  problem.addVariable('role', ['TOP', 'JGL', 'MID', 'BOT', 'SUP'])

  def test_constraint(
    selection: str,
    role: list
  ):
    if selection in champions:
      return True

  problem.addConstraint(test_constraint, ['selection'])
